...
If you use the Qt version, you need to execute `uv run pyinstaller -F --noconsole --name "CCFWF-Compressor-Qt" --add-data "fonts;fonts;" floppyCompress.py` / `uv run pyinstaller -F --noconsole --name "CCFWF-Extractor-Qt" --add-data "fonts;fonts;" floppyExtract.py`. 
...
- A maximum of 99,999 files can be generated; exceeding this limit may result in program errors or unpredictable behavior. Based on the provided options, the 1440KB setting supports a maximum file size of approximately 1.07 TB.
